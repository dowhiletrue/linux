...
#ifdef LCD_DEBUG
static ssize_t lcd_read(struct device *dev,
...
//	ref_clk = clk_get_rate(clk);
	ref_clk = clk_round_rate(clk, needed_pixclk);
...
static void set_dma_control(struct dove_lcd *dove_lcd)
{
	...
	DRM_DEBUG_DRIVER("fmt %.4s\n",
				(char *) dove_lcd->crtc.primary->fb->pixel_format);
